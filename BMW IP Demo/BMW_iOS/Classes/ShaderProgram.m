...
+ (ShaderProgram*)programWithVertexShader:(NSString*)a_vertShader andFragmentShader:(NSString*)a_fragmentShader {
    ...
    if(toReturn == nil) {
		...
		result = [toReturn loadUniformMap];
...
- (BOOL)loadUniformMap {
    GLint numToRead = 0;
    glGetProgramiv(m_programID, GL_ACTIVE_UNIFORMS, &numToRead);
    char nameBuf[255];
    GLsizei len;
    GLint size;
    GLenum type;
    for(int i = 0; i < numToRead; i++) {
        bzero(nameBuf, sizeof(char) * 255);
        glGetActiveUniform(m_programID, i, 255, &len, &size, &type, nameBuf);
        GLint loc = glGetUniformLocation(m_programID, nameBuf);
        NSString* uniformName = [NSString stringWithCString:nameBuf encoding:NSASCIIStringEncoding];
        if(sm_showDebugging) {
            NSLog(@"\tUniform \"%@\" using index: %d", uniformName, loc);
        }
        [m_uniformMap setObject:[NSNumber numberWithInt:loc] forKey:uniformName];
    }
    return YES;
}
